...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Crossword Streak")
    ...
    private func entryRow(_ entry: PuzzleEntry) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.title)
                .font(Theme.headingFont)
                .foregroundColor(Theme.textPrimary)
            Text(String(describing: entry.source))
...
struct AddEntrySheet: View {
    ...
    @State private var draftSolveDate: Date = Date()
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Source", text: $draftSource)
                        .accessibilityIdentifier("field_source")
                    DatePicker("Date solved", selection: $draftSolveDate, displayedComponents: .date)
                        .accessibilityIdentifier("field_solveDate")
    ...
    private func save() {
        if var existing = entry {
            ...
            let newEntry = PuzzleEntry(title: title, rating: rating, source: draftSource, solveDate: draftSolveDate, minutes: draftMinutes)
            store.add(newEntry)
        }
        draftSource = ""
        draftSolveDate = Date()
